...
function getSql() {
  ...
  if (!url) throw new Error("DATABASE_URL is not set. Add it from the Neon dashboard.");
  return neon(url);
}
...
export async function publishTimelineAction(formData: FormData): Promise<string> {
  const sql = getSql();
  const token = process.env.BLOB_READ_WRITE_TOKEN;
  if (!token) {
    throw new Error("BLOB_READ_WRITE_TOKEN is not set. Create a Blob store on Vercel (or set the token).");
  }
  ...
  const slug = nanoid(8);
  const prefix = `letter-through-time/${slug}`;
